...
Status: Native baseline verified; founder review and CI container evidence
pending
...
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| Architecture catalogue             | Passed: 20 components and 39 contract families                                                                     | Catalogue consistency, not implementation of every component                                              |
| Common contract validation         | Passed: 6 JSON Schema 2020-12 schemas, 12 positive/negative fixtures and 6 compatibility descriptors               | Contracts remain working drafts pending founder review                                                    |
...
| OCI container execution            | Not run locally: no OCI runtime is installed in this session                                                       | CI now builds the image and repeats the accepted/duplicate test across two containers using one volume    |
...
M1 remains `in-development` until:

1. the founders review the framework security model, threat model, common
   contracts and proposed ADRs `0004` to `0006`;
2. repository CI confirms the Linux and OCI-container gates from a committed
   revision; and
3. any review finding is resolved or explicitly recorded as a later-milestone
   limitation.
