...
impl IamRuntime {
    ...
    pub fn bootstrap_local(
        ...
        let result = self.bootstrap_local_locked(now);
    ...
    fn bootstrap_local_locked(
        ...
        let environment_id = random_id("env", 16)?;
...
fn canonical_claims(claims: &DemonstrationSignInGrantClaims) -> Vec<u8> {
...
fn random_id(prefix: &str, length: usize) -> Result<String, IamError> {
