...
helm upgrade --install nats nats/nats \
  ...
  --set-json 'config.merge.authorization.users[0].permissions.publish=["ppl.m3.to-presentation.*","ppl.m3.events.director","$JS.>"]' \
...
kubectl --namespace "$namespace" rollout restart \
  deployment/m3-scenario-director deployment/m3-presentation-gateway
...
for workload in m3-scenario-director m3-presentation-gateway; do
...
printf '%s\n' \
  "M3.3 Minikube image: $image_digest" \
