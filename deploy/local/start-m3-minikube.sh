...
helm upgrade --install nats nats/nats \
  ...
  --set-json 'config.merge.authorization.users[1].permissions.publish=["ppl.m3.to-director.*","ppl.gate-a.events.CTL-02","$JS.>"]' \
  ...
  --set-json 'config.merge.authorization.users[5].permissions.publish=["ppl.gate-a.events.CNT-01"]' \
  --set-json 'config.merge.authorization.users[5].permissions.subscribe=["ppl.gate-a.commands.CNT-01","_INBOX.>"]' \
